...
## Yêu cầu máy build
- Windows 10/11 x64
- Python 3.13 + venv ở `backend/venv` (đã cài `requirements.lock.txt` — có torch bản cu124 cho OmniVoice)
...
Chỉ có **một bản build duy nhất** — bản **FULL**, nhúng sẵn cả 2 engine TTS: VBEE (cloud) + OmniVoice (local). Gói nặng ~6–10GB.
...
> **OmniVoice chạy trên cả GPU lẫn CPU.** Máy có GPU NVIDIA → chạy nhanh; máy không có GPU → app **tự động chuyển sang CPU** (`effective_device()` trong `omnivoice_processor.py`), vẫn chạy được chỉ chậm hơn (~15–20× so với realtime). Không bắt buộc phải có GPU khi cài.
...
## Ghi chú
- Bản build luôn là bản FULL (nhúng cả VBEE + OmniVoice). Người dùng chọn engine nào để dùng ngay trong app.
- OmniVoice chạy CPU khi không có GPU; có thể ép chế độ CPU/GPU qua setting `OMNIVOICE_USE_CPU` (mặc định auto-detect theo phần cứng).
