...
"""
PyInstaller spec for TruyenFull Processor — GPU build WITH embedded OmniVoice
local TTS (torch + CUDA + transformers). Large (~6-10 GB onedir).

Build from the repo root (venv must have requirements.lock.txt installed):
    backend/venv/Scripts/pyinstaller.exe packaging/truyenfull-gpu.spec --noconfirm

Produces dist/TruyenFullProcessor/TruyenFullProcessor.exe (onedir).

For the lightweight VBEE-only build, use truyenfull.spec instead.
"""
import os
